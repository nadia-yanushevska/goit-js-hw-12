...
`)}function R(e){const{webformatURL:s,largeImageURL:r,previewHeight:n,previewWidth:t,likes:o,views:a,comments:v,downloads:S,tags:E}=e;return`<li class="gallery-item">
            ...
            </li>`}function x(e){return e.split(" ").filter(r=>r).join("+")}function m(e){e.classList.remove("is-hidden")}function f(e){e.classList.add("is-hidden")}const C="Sorry, there are no images matching your&nbsp;search query. Please, try again!",M=`"We're sorry, but you've reached the end of search results."`;let i,p,l=1;const g=document.querySelector(".gallery"),T=document.querySelector("[js-form]"),u=document.querySelector("[js-load-btn]"),h=document.querySelector("[js-loader]");T.addEventListener("submit",U);u.addEventListener("click",A);document.addEventListener("wheel",L);function U(e){e.preventDefault(),m(h),g.innerHTML="",i=x(e.target.elements["search-name"].value),p!==i?l=1:l++,b(i,l),p=i,e.target.reset()}async function A(){m(h),await b(i,++l),L({deltaY:10})}async function b(e,s=1){f(u);try{const n=(await H(e,s)).data,t=[...n.hits],o=n.totalHits;t.length===0?c(C):(O(g,t),o>s*w?m(u):c(M))}catch(r){c(r)}finally{f(h)}}function L(e){const s=e.deltaY>0?2:-2,r=g.querySelector(".gallery-item"),n=Number.parseInt(r.getBoundingClientRect().height),t={top:s*n,behavior:"smooth"};window.scrollBy(t)}
