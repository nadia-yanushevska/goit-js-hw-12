...
`)}function R(e){const{webformatURL:s,largeImageURL:r,previewHeight:n,previewWidth:t,likes:o,views:a,comments:v,downloads:S,tags:E}=e;return`<li class="gallery-item">
            ...
            </li>`}function x(e){return e.split(" ").filter(r=>r).join("+")}function f(e){e.classList.remove("is-hidden")}function g(e){e.classList.add("is-hidden")}const C="Sorry, there are no images matching your&nbsp;search query. Please, try again!",D=`"We're sorry, but you've reached the end of search results."`;let i,h,l=1;const m=document.querySelector(".gallery"),M=document.querySelector("[js-form]"),u=document.querySelector("[js-load-btn]"),p=document.querySelector("[js-loader]");M.addEventListener("submit",T);u.addEventListener("click",U);document.addEventListener("wheel",L);function T(e){e.preventDefault(),f(p),m.innerHTML="",i=x(e.target.elements["search-name"].value),h!==i?l=1:l++,b(i,l),h=i,e.target.reset()}async function U(){f(p),await b(i,++l),L({deltaY:10,preventDefault(){}})}async function b(e,s=1){g(u);try{const n=(await H(e,s)).data,t=[...n.hits],o=n.totalHits;t.length===0?c(C):(O(m,t),o>s*w?f(u):c(D))}catch(r){c(r)}finally{g(p)}}function L(e){e.preventDefault();const s=e.deltaY>0?2:-2,r=m.querySelector(".gallery-item"),n=Number.parseInt(r.getBoundingClientRect().height),t={top:s*n,behavior:"smooth"};window.scrollBy(t)}
